...
public class Utilz {
                ...
                if (testTile.getyCord() < lvlTypeArr.length)
                    if (testTile.getxCord() >= 0)
                        if (testTile.getxCord() < lvlTypeArr[0].length)
                            if (lvlTypeArr[testTile.getyCord()][testTile.getxCord()] == ROAD)
                                return true;
    ...
    public static int GetDistance(float x1, float y1, float x2, float y2) {  //calculates distance
        float xDiff = Math.abs(x1 - x2);
        float yDiff = Math.abs(y1 - y2);

        return (int)Math.hypot(xDiff, yDiff); //cast to int, bcs method return double
    }
}
